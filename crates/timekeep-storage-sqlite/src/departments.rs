...
impl SqliteStorage {
    ...
    pub(super) async fn get_department(&self, id: &str) -> Result<Option<Department>, Error> {
        let row = sqlx::query_as::<_, DepartmentRow>(
            "SELECT id, name, work_policy_id, work_policy_json, created_at, updated_at FROM departments WHERE id = ?",
        )
        .bind(id)
    ...
    pub(super) async fn get_department_by_name(
        ...
        let row = sqlx::query_as::<_, DepartmentRow>(
            "SELECT id, name, work_policy_id, work_policy_json, created_at, updated_at FROM departments WHERE name = ?",
        )
        .bind(name)
        .fetch_optional(&self.pool)
        .await
    ...
    pub(super) async fn update_department(&self, dept: &Department) -> Result<(), Error> {
        ...
        sqlx::query(
            "UPDATE departments SET name = ?, work_policy_id = ?, work_policy_json = ?, updated_at = ? WHERE id = ?",
        )
        .bind(&dept.name)
        .bind(&dept.work_policy_id)
        .bind(&policy_json)
