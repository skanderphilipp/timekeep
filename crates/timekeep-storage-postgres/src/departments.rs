...
impl PostgresStorage {
    ...
    pub(super) async fn get_department(&self, id: &str) -> Result<Option<Department>, Error> {
        let row = sqlx::query_as::<_, DepartmentRowPg>(
            "SELECT id, name, work_policy_json, created_at, updated_at FROM departments WHERE id = $1",
        )
        .bind(id)
    ...
    pub(super) async fn get_department_by_name(
        ...
        let row = sqlx::query_as::<_, DepartmentRowPg>(
            "SELECT id, name, work_policy_json, created_at, updated_at FROM departments WHERE name = $1",
        )
        .bind(name)
        .fetch_optional(&self.pool)
        .await
    ...
    pub(super) async fn update_department(&self, dept: &Department) -> Result<(), Error> {
        ...
        sqlx::query(
            "UPDATE departments SET name = $1, work_policy_json = $2, updated_at = $3 WHERE id = $4",
        )
        .bind(&dept.name)
        .bind(&policy_json)
        .bind(dept.updated_at.as_second())
