...
const isFile = (segment: string, isLastSegment: boolean): boolean => isLastSegment;

const validateSegment = (
  segment: string,
  context: PathContext,
  isLastSegment: boolean,
): { nextContext: PathContext; error?: undefined } | { error: ValidationError } => {
  switch (context.type) {
    case "modules_root": {
 ...
 */
const extractModulesRelativePath = (filename: string, marker: string): string | null => {
...
export const rule = defineRule({
  meta: {
    ...
    messages: {
      ...
      moduleNameNotKebabCase: "Module folder '{{ name }}' must be kebab-case (e.g. 'my-module').",
      moduleTooDeep: "Module folder '{{ name }}' exceeds maximum nesting depth of {{ max }}.",
      hookFileNaming:
        "Hook file '{{ name }}' must match use-kebab-case.ts (e.g. 'use-my-hook.ts').",
      hooksInternalTooDeep: "hooks/internal/ nesting exceeds maximum depth of {{ max }}.",
      invalidHooksEntry:
        "Unexpected entry '{{ name }}' in hooks/. Only hook files, __tests__/, __mocks__/, and internal/ are allowed.",
      utilFileNaming: "Util file '{{ name }}' must match {camelCase}.(ts|tsx) (e.g. 'myUtil.ts').",
      invalidUtilsEntry:
    ...
    return {
      Program: (node: any) => {
        ...
        for (let i = 0; i < segments.length; i++) {
          ...
          const result = validateSegment(segment, currentContext, isLastSegment);
