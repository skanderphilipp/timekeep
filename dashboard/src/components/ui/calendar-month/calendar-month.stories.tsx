...
import { CalendarMonth, type CalendarDayStatus } from "./calendar-month";

/**
 * Generate mock attendance data for a month.
 * 70% full, 10% half, 8% late, 5% absent, rest weekend.
 */
function mockDayStatus(year: number, month: number): Record<string, { status: CalendarDayStatus; hours?: number | null }> {
  ...
  render: () => {
    ...
    for (let d = 1; d <= 31; d++) {
      ...
      if (isWeekend) {
        ...
        else if (roll < 0.50) bad[key] = { status: "late", hours: 7.5 };
