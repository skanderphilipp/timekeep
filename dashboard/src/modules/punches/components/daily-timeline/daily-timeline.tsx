...
import { computeAttendanceSummary, formatDuration } from "./attendance-summary";
...
export function DailyTimeline({ date, filterSince, filterUntil, employees, className }: DailyTimelineProps) {
	const { _ } = useLingui();
	const openSidePanel = useSetAtom(openSidePanelAtom);

	// Use explicit filter range when provided (syncs with parent page filters).
	// Falls back to the single `date` prop for backward compatibility.
	const since = useMemo(
